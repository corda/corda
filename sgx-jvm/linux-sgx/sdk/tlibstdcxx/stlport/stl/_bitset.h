...
//
_STLP_TEMPLATE_NULL
struct _Base_bitset<0UL> {
  typedef unsigned long _WordT;
  typedef _Base_bitset<0UL> _Self;
  ...
  _WordT& _M_getword(size_t) {
    __stl_throw_out_of_range("_Base_bitset::_M_getword"); 
    return *new _WordT;
  }
  _WordT _M_getword(size_t) const { return 0; }
  ...
  bool _M_is_equal(const _Self&) const { return true; }
  bool _M_is_any() const { return false; }

  size_t _M_do_count() const { return 0; }
  unsigned long _M_do_to_ulong() const { return 0; }
  size_t _M_do_find_first(size_t) const { return 0; }

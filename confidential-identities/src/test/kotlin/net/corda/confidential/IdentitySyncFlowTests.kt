...
class IdentitySyncFlowTests {
    ...
    fun `sync confidential identities`() {
        ...
        val issueFlow = aliceNode.services.startFlowAndReturnFuture(CashIssueAndPaymentFlow(1000.DOLLARS, ref, alice, anonymous, notary))
        ...
        aliceNode.services.startFlowAndReturnFuture(Initiator(bob, issueTx.tx)).getOrThrow()
    ...
    fun `don't offer other's identities confidential identities`() {
        ...
        val anonymous = true
        val ref = OpaqueBytes.of(0x01)
        val issueFlow = charlieNode.services.startFlowAndReturnFuture(CashIssueAndPaymentFlow(1000.DOLLARS, ref, charlie, anonymous, notary))
        val issueTx = issueFlow.getOrThrow().stx
        val confidentialIdentity = issueTx.tx.outputs.map { it.data }.filterIsInstance<Cash.State>().single().owner
        ...
        val payTx = charlieNode.services.startFlowAndReturnFuture(CashPaymentFlow(1000.DOLLARS, alice, anonymous)).getOrThrow().stx

        // Run the flow to sync up the identities, and confirm Charlie's confidential identity doesn't leak
        assertNull(bobNode.database.transaction { bobNode.services.identityService.wellKnownPartyFromAnonymous(confidentialIdentity) })
        aliceNode.services.startFlowAndReturnFuture(Initiator(bob, payTx.tx)).getOrThrow()
        assertNull(bobNode.database.transaction { bobNode.services.identityService.wellKnownPartyFromAnonymous(confidentialIdentity) })
    }

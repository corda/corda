...
class NodeMonitorModelTest {
    ...
    private fun setup(startNodesInProcess: Boolean = true, runTest: () -> Unit) {
        internalDriver(startNodesInProcess = startNodesInProcess, extraCordappPackagesToScan = listOf("net.corda.finance")) {
    ...
    fun `network map update`() {
        setup(startNodesInProcess = false) {
                    ...
                    .expectEvents(isStrict = false) {
                        sequence(
                                ...
                                expect { output: NetworkMapCache.MapChange ->
                                    require(output.node.legalIdentities.any { it.name == ALICE_NAME }) {
                                        "Expecting : $ALICE_NAME, Actual : ${output.node.legalIdentities.map(Party::name)}"
                                    }
                                },
                                expect { output: NetworkMapCache.MapChange ->
                                    require(output.node.legalIdentities.any { it.name == BOB_NAME }) {
                                        "Expecting : $BOB_NAME, Actual : ${output.node.legalIdentities.map(Party::name)}"
                                    }
                                },
                                expect { output: NetworkMapCache.MapChange ->
                                    require(output.node.legalIdentities.any { it.name == CHARLIE_NAME }) {
                                        "Expecting : $CHARLIE_NAME, Actual : ${output.node.legalIdentities.map(Party::name)}"
                                    }
                                }
                        )
                    }

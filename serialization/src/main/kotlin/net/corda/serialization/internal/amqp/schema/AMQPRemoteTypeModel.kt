...
import net.corda.core.internal.reflection.*
...
class AMQPRemoteTypeModel {
    ...
    data class InterpretationState(val notationLookup: Map<TypeIdentifier, TypeNotation>,
        ...
        private fun RestrictedType.interpretEnum(identifier: TypeIdentifier): RemoteTypeInformation.AnEnum {
            ...
            val transforms = try {
                enumTransformsLookup[identifier]?.let { EnumTransforms.build(it.toLocal(), constants) } ?: EnumTransforms.empty
...
private fun TransformsMap.toLocal(): LocalTransformsMap = asSequence().map { (k, v) ->
    k.toLocal() to v.map { it.toLocal() }.toMutableList()
}.toMap(EnumMap<LocalTransformTypes, MutableList<LocalTransform>>(LocalTransformTypes::class.java))

private fun TransformTypes.toLocal(): LocalTransformTypes = when(this) {
    TransformTypes.Unknown -> LocalTransformTypes.Unknown
    TransformTypes.EnumDefault -> LocalTransformTypes.EnumDefault
    TransformTypes.Rename -> LocalTransformTypes.Rename
}

private fun Transform.toLocal(): LocalTransform = when(this) {
    is UnknownTransform -> LocalTransform.UnknownTransform
    is EnumDefaultSchemaTransform -> LocalTransform.EnumDefaultSchemaTransform(old = old, new = new)
    is RenameSchemaTransform -> LocalTransform.RenameSchemaTransform(from = from, to = to)
    else -> throw UnsupportedOperationException("Cannot find a LocalTransform equivalent to $this")
}
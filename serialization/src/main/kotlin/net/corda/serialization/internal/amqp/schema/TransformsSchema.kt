...
import net.corda.core.internal.reflection.LocalTransform
import net.corda.core.internal.reflection.LocalTransformTypes
...
fun LocalTransform.toAMQP(): Transform = when(this) {
    is LocalTransform.UnknownTransform -> UnknownTransform()
    is LocalTransform.EnumDefaultSchemaTransform -> EnumDefaultSchemaTransform(old = old, new = new)
    is LocalTransform.RenameSchemaTransform -> RenameSchemaTransform(from = from, to = to)
}

fun LocalTransformTypes.toAMQP(): TransformTypes = when(this) {
    LocalTransformTypes.Unknown -> TransformTypes.Unknown
    LocalTransformTypes.EnumDefault -> TransformTypes.EnumDefault
    LocalTransformTypes.Rename -> TransformTypes.Rename
}
            ...
            val transformsMap = schema.types.asSequence().mapNotNull { type ->
                ...
                if (localTypeInformation is LocalTypeInformation.AnEnum) {
                    localTypeInformation.transforms.source.let {
                        if (it.isEmpty()) null else type.name to it.asSequence().map { (k, v) ->
                            k.toAMQP() to v.map { it.toAMQP() }.toMutableList()
                        }.toMap(EnumMap<TransformTypes, MutableList<Transform>>(TransformTypes::class.java))
                    }
                }
                else null
            }.toMap()

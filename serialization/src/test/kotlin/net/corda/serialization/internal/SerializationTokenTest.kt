...
class SerializationTokenTest {
    ...
    private fun serializeAsTokenContext(toBeTokenized: Any) = context.withTokenContextTokenizing(toBeTokenized, rigorousMock())
    @Test
    fun `write token and read tokenizable`() {
        val tokenizableBefore = LargeTokenizable()
        val testContext = serializeAsTokenContext(tokenizableBefore)
    ...
    fun `new token encountered after context init`() {
        val tokenizableBefore = UnitSerializeAsToken()
        val testContext = serializeAsTokenContext(emptyList<Any>())
    ...
    fun `deserialize unregistered token`() {
        val tokenizableBefore = UnitSerializeAsToken()
        val testContext = serializeAsTokenContext(emptyList<Any>())
        val serializedBytes = tokenizableBefore.toToken(
                context.createSerializeAsTokenContext(emptyList<Any>(), rigorousMock()))
                .checkpointSerialize(testContext)
    ...
    fun `deserialize non-token`() {
        ...
        Output(stream).use {
            KryoCheckpointSerializer.kryoMagic.writeTo(it)

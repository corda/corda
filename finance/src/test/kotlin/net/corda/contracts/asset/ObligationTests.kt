...
class ObligationTests {
    ...
    val trustedCashContract = linkedSetOf(SecureHash.randomSHA256() as SecureHash)
    val megaIssuedDollars = linkedSetOf(Issued(defaultIssuer, USD))
    val megaIssuedPounds = linkedSetOf(Issued(defaultIssuer, GBP))
    ...
    fun `commodity settlement`() {
        ...
        val obligationDef = Obligation.Terms(linkedSetOf(CommodityContract().legalContractReference), linkedSetOf(defaultFcoj), TEST_TX_TIME)
    ...
    fun `nettability of settlement contracts`() {
        ...
        assertNotEquals(fiveKDollarsFromMegaToMega.bilateralNetState,
                fiveKDollarsFromMegaToMega.copy(template = megaCorpDollarSettlement.copy(acceptableContracts = linkedSetOf(SecureHash.randomSHA256()))).bilateralNetState)

        // States must not be nettable if the trusted issuers differ
        val miniCorpIssuer = linkedSetOf(Issued(MINI_CORP.ref(1), USD))
        assertNotEquals(fiveKDollarsFromMegaToMega.bilateralNetState,
    ...
    val Issued<Currency>.OBLIGATION_DEF: Obligation.Terms<Currency>
        get() = Obligation.Terms(linkedSetOf(Cash().legalContractReference), linkedSetOf(this), TEST_TX_TIME)

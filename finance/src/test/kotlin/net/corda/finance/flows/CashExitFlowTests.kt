...
class CashExitFlowTests {
    ...
    fun start() {
        ...
        val future = bankOfCordaNode.services.startFlowAndReturnFuture(CashIssueFlow(initialBalance, ref, notary))
        mockNet.runNetwork()
        future.getOrThrow()
    }
    ...
    fun `exit some cash`() {
        ...
        val future = bankOfCordaNode.services.startFlowAndReturnFuture(CashExitFlow(exitAmount, ref))
        mockNet.runNetwork()
    ...
    fun `exit zero cash`() {
        ...
        val future = bankOfCordaNode.services.startFlowAndReturnFuture(CashExitFlow(expected, ref))
        mockNet.runNetwork()

...
class CommercialPaperTestsGeneric {
    ...
    fun `issue move and then redeem`() = withTestSerialization {
        ...
        databaseAlice.transaction {
            alicesVault = VaultFiller(aliceServices, Pair(DUMMY_NOTARY, DUMMY_NOTARY_KEY), rngFactory = ::Random).fillWithSomeTestCash(9000.DOLLARS, issuerServices, 1, DUMMY_CASH_ISSUER)
            aliceVaultService = aliceServices.vaultService
        }
        ...
        databaseBigCorp.transaction {
            bigCorpVault = VaultFiller(bigCorpServices, Pair(DUMMY_NOTARY, DUMMY_NOTARY_KEY), rngFactory = ::Random).fillWithSomeTestCash(13000.DOLLARS, issuerServices, 1, DUMMY_CASH_ISSUER)
            bigCorpVaultService = bigCorpServices.vaultService
        }

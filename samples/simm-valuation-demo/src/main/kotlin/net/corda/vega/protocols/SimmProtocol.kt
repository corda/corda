...
/**
 * The Simm Protocol is between two parties that both agree on a portfolio of trades to run valuations on. Both sides
...
object SimmProtocol {
    ...
    : ProtocolLogic<RevisionedState<PortfolioState.Update>>() {
        ...
        private fun agreePortfolio(portfolio: Portfolio) {
            ...
            subProtocol(TwoPartyDealProtocol.Acceptor(otherParty), shareParentSessions = true)
        }
        ...
        private fun updatePortfolio(portfolio: Portfolio, stateAndRef: StateAndRef<PortfolioState>) {
            ...
            subProtocol(StateRevisionProtocol.Requester(stateAndRef, update), shareParentSessions = true)
        }
        ...
        private fun updateValuation(stateRef: StateAndRef<PortfolioState>): RevisionedState<PortfolioState.Update> {
            ...
            return subProtocol(StateRevisionProtocol.Requester(stateRef, update), shareParentSessions = true).state.data
        }
    ...
    /**
     * Service plugin for listening for incoming Simm protocol communication
     */
     ...
     */
    class Receiver(val replyToParty: Party) : ProtocolLogic<Unit>() {
        ...
        override fun call() {
            ...
            logger.info("SimmProtocol receiver started")
        ...
        /**
         * So this is the crux of the Simm Agreement protocol
         ...
         * 7) Now the sensitivities can be agreed over the protocol
        ...
        private fun agreePortfolio(portfolio: Portfolio): SignedTransaction {
            ...
            val seller = TwoPartyDealProtocol.Instigator(
                    replyToParty,
                    TwoPartyDealProtocol.AutoOffer(offer.notary, offer.dealBeingOffered),
                    serviceHub.legalIdentityKey)
            logger.info("Starting two party deal initiator with: ${replyToParty.name}")
            return subProtocol(seller, shareParentSessions = true)
        }
        ...
        private fun updatePortfolio(portfolio: Portfolio) {
            ...
            subProtocol(StateRevisionProtocol.Receiver<PortfolioState.Update>(replyToParty, {

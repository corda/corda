...
class TraderDemoTest {
    ...
    fun `runs trader demo`() {
        ...
        driver(startNodesInProcess = true, extraCordappPackagesToScan = listOf("net.corda.finance")) {
            ...
            ).map { (it.getOrThrow() as NodeHandle.InProcess) }
            nodeA.registerInitiatedFlow(BuyerFlow::class.java)
            val (nodeARpc, nodeBRpc) = listOf(nodeA, nodeB).map {
                val client = CordaRPCClient(it.configuration.rpcOptions.address!!)
                client.start(demoUser.username, demoUser.password).proxy
            }
            val nodeBankRpc = let {
                val client = CordaRPCClient(bankNode.configuration.rpcOptions.address!!)

...
Corda has custom restrictions on the format of certificate subject names for node CA and below, see
:doc:`Distinguished Names <permissioning-node-name>`. A custom extension is used to describe
the role of a certificate; this is provided automatically for certificates generated by Corda, but the extension is
documented at :doc:`Certificate Specification <permissioning-certificate-specification` for reference if externally
generating certificates for use with Corda.

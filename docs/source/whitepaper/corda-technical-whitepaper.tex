...
that state as a non-consumed input. Note that the `latest' version is determined relative to an individual node's
...
suited to representing value-oriented agreements. Despite the apparent similarity tokens are not cryptocurrencies.
All tokens in Corda ultimately represent claims upon something.

Tokens can be used to model money, securities, derivatives, debts and other financial instruments. They could also
be used to model any sort of claim on physical resources, like CPU time, network bandwidth, barrels of oil and so
on.

Note that cryptocurrencies are not tokens, in Corda's terminology. Cryptocurrencies are different to almost
anything else you may model on a distributed ledger, because (native) cryptocurrencies do \emph{not} represent a
claim on an issuer - mere possession of the relevant database entry is, itself, the entire value.
...
On top of these universal core types, Corda provides a dedicated `token software development kit' module that
extends the type system with more sophisticated concepts.
...
(see~\cref{subsec:transaction-structure}).
...
algorithms for this task under this name.

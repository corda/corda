Advanced: Running the demos
===========================

.. contents::

The `Corda repository <https://github.com/corda/corda>`_ contains a number of demo programs demonstrating
Corda's functionality. The demos listed below demonstrate more complex functionality and are primarily intended for
use in integration testing by Corda core developers.

You should be familiar with the standard demos described in :doc:`running the demos <running-the-demos>` before reading
this document.

1. The :ref:`irs-demo`, which shows two nodes establishing an interest rate swap and performing fixings with a
   rates oracle
2. The :ref:`notary-demo`, which shows three different types of notaries and a single node getting multiple transactions
   notarised
3. The :ref:`simm-demo`, which shows agreement between a set of nodes on initial margin rates.

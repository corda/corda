...
Corda's embedded web server ``corda-webserver.jar`` requires its own configuration file ``web-server.conf``.

Format
------

Corda's embedded web server uses the same format of ``node.conf`` property file.

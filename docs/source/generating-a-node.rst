...
By default all Cordapp JARs are signed. The keystore is generated in `./build/nodes/` directory.
...
* ``generateKeystore`` - if set to ``true`` (by default) the task will generate a key store if existing is not present, the key/keystore will have default values if not set in ``options``, the flag may be permanently set to ``true`` because the task doesn't overwrite an existing keyStore
* ``options`` - any relevant task parameters as for ANT tasks `GenKey <https://ant.apache.org/manual/Tasks/genkey.html>`_ and `SignJar <https://ant.apache.org/manual/Tasks/signjar.html>`_ , the minimal set of options to sign using external key store (not generated by the task):

    * ``keystore`` - path to the keystore file, the default path for generated keystore is `./build/nodes/jarSignKeystore.p12`
    * ``alias`` - the alias to sign under, the default value is `cordapp-signer`
    * ``storepass`` - password for keystore, the default value is `secret1!`
    * ``keypass``  - password for private key if different than password for keystore
    * ``storetype`` - keystore type, the default value is `PKCS12`
    For the keystore generation the additional options are needed:

    * ``dname`` - the distinguished name for entity, the default value is `OU=Dummy Cordapp Distributor, O=Corda, L=London, C=GB`

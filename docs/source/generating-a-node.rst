...
Signing the Cordapp enables it's contract classes to use signature constraints instead of other types of the constraints :doc:`api-contract-constraints`.
...
* ``options`` any relevant task parameters as for ANT tasks  `SignJar <https://ant.apache.org/manual/Tasks/signjar.html>`_ and `GenKey <https://ant.apache.org/manual/Tasks/genkey.html>`_,
  by default JAR is signed by Corda development key, the external keystore can be specified, the minimum set of options listed below (for all options referer to the SignJar ANT task description).
  If ad-hock keystore is generated (``generateKeystore`` set to `true``) it can be further configured by ``dname`` and ``keyalg`` options (and other options for the GenKey ANT task).
  the minimal set of options to sign using an external keystore (not generated by the task):

  * ``keystore`` path to the keystore file, by default cordadevcakeys.jks keyStore ìs shipped with the plugin
  ...
  * ``dname`` the distinguished name for entity, used for keystore generation only
  * ``keyalg`` the method to use when generating name-value pair, used for keystore generation only, Corda doesn't support `DSA`, the value defaults to `RSA`

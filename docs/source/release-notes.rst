...
As such we'd like to extend special thanks to
  ...
  * Ben Wyeth got providing a mechanism for registering a callback on app shutdown
  * Tomas Tauber for adding support for running the vault service using PostgresSQL in place of in memory H2 service
...
* **Wire Stability**:
  ...
  remain able to always read States generated by older and newer versions of itself. This allows system to be deployed safe in the
  knowledge that valuable and important information will always be accessible through upgrade and change.

  * **R3 Corda 3.0 compaitibility**
    ...
    nodes can interoperate without issue.
  ...
  * **AMQP Serialization**
    ...
    .. note:: This release delivers the bulk of our transition from Kryo serialisation to AMQP serialisation. This means that many of the restrictions
      ...
      the restrictions documented at the link above are the canonical source.0

  * **Artemis and Bridges**
    This is good because... <<< Mathew >>>
...
* **Security Auditing**
  ...
  This verions of Corda is the first to have been throughly internally audited by R3's internal security team. In addition
  several external security audits of the code have been undertaken. In all cases the reports have been... <<< JAMES BROWN >>>

* **Contract Upgrade**

  <<< Tudor >>> Is awesome because... see documentation here...

  .. note:: be aware of XXX
...
* **Configurable authorization and authentication data sources**
    ...
    Apace Shiro Modular Crypt Format encoded passwords
...
~~~~~~~~~~~~~
  ...
  * Flows may now be started via a service
  * Database tables have a more consistent naming scheme
  * The Vault Query API has been unified with the Vault Service API to create a single Vault Service API
  * RPC calls can now work over SSL
  * Multiple Identity support for cash spending
  * TLS authentication now supports mixed RSA and ECDSA keys
  * Pluggable serializer framework for external classes
  * A number of identified performance ssues have been improved
  * Numerous bug fixes and documentation tweeks

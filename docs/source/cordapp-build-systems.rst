...
a value for ``alias`` can be taken from the ``signingAlias`` system property. The following system properties can be used:
...
If the option is not set, the relevant system property named `signing[OptionName]` is tried.

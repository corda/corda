...
To test an implicit upgrade, you must simulate a network that initially has only nodes with the old version, and then nodes gradually transition to the new version.
...
4. Upgrade a couple of nodes to the new version.
...
are  being successfully read by old nodes.

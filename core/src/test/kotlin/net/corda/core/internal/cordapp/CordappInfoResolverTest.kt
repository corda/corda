...
class CordappInfoResolverTest {
    ...
    fun `the correct cordapp resolver is used after calling withCordappInfo`() {
        ...
        CordappInfoResolver.register(listOf(javaClass.name), Default("test", "test", "2", 3, defaultTargetVersion))
    ...
    fun `when more than one cordapp is registered for the same class, the resolver returns null`() {
        CordappInfoResolver.register(listOf(javaClass.name), Default("test", "test", "2", 3, 222))
        CordappInfoResolver.register(listOf(javaClass.name), Default("test1", "test1", "1", 2, 456))

...
class ContractUpgradeFlowTest {
    ...
    fun `2 parties contract upgrade`() {
        ...
        aliceNode.services.startFlowAndReturnFuture(FinalityFlow(stx, setOf(bob)))
        mockNet.runNetwork()
        ...
        val rejectedFuture = aliceNode.services.startFlowAndReturnFuture(ContractUpgradeFlow.Initiate(atx!!.tx.outRef(0), DummyContractV2::class.java))
        mockNet.runNetwork()
        ...
        bobNode.services.startFlowAndReturnFuture(ContractUpgradeFlow.Authorise(btx!!.tx.outRef<ContractState>(0), DummyContractV2::class.java)).getOrThrow()
        bobNode.services.startFlowAndReturnFuture(ContractUpgradeFlow.Deauthorise(btx.tx.outRef<ContractState>(0).ref)).getOrThrow()

        // The request is expected to be rejected because party B has subsequently deauthorised and a previously authorised upgrade.
        val deauthorisedFuture = aliceNode.services.startFlowAndReturnFuture(ContractUpgradeFlow.Initiate(atx.tx.outRef(0), DummyContractV2::class.java))
        mockNet.runNetwork()
        ...
        val resultFuture = aliceNode.services.startFlowAndReturnFuture(ContractUpgradeFlow.Initiate(atx.tx.outRef(0), DummyContractV2::class.java))
        mockNet.runNetwork()
    ...
    fun `upgrade Cash to v2`() {
        ...
        val result = aliceNode.services.startFlowAndReturnFuture(CashIssueFlow(Amount(1000, USD), OpaqueBytes.of(1), notary))
        mockNet.runNetwork()
        ...
        val upgradeResult = aliceNode.services.startFlowAndReturnFuture(ContractUpgradeFlow.Initiate(stateAndRef, CashV2::class.java))
        mockNet.runNetwork()

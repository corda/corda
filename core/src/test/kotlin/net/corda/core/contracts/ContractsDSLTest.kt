...
class ContractsDSLTest {
    ...
    class RequireSingleCommandTest(private val testFunction: (Collection<CommandWithParties<CommandData>>) -> CommandWithParties<CommandData>, description: String) {
        companion object {
            @JvmStatic
        ...
        fun `check error is thrown when command is of wrong type`() {
            ...
            Assertions.assertThatThrownBy { testFunction(commands) }
                    ...
                    .hasMessage("Required net.corda.core.contracts.ContractsDSLTest.TestCommands command")
        }
    }

    @RunWith(Parameterized::class)
    class SelectWithSingleInputsTest(private val testFunction: (Collection<CommandWithParties<CommandData>>, PublicKey?, AbstractParty?) -> Iterable<CommandWithParties<CommandData>>, description: String) {
        companion object {
            @JvmStatic
            @Parameterized.Parameters(name = "{1}")
            fun data(): Collection<Array<Any>> = listOf(
    ...
    @RunWith(Parameterized::class)
    class SelectWithMultipleInputsTest(private val testFunction: (Collection<CommandWithParties<CommandData>>, Collection<PublicKey>?, Collection<Party>?) -> Iterable<CommandWithParties<CommandData>>, description: String) {
        companion object {
            @JvmStatic
            @Parameterized.Parameters(name = "{1}")
            fun data(): Collection<Array<Any>> = listOf(

...
class KryoTests {
    ...
    fun `serialize - deserialize X509Certififcate`() {
        val (expected, _) = X509Utilities.createSelfSignedCACert(ALICE.name, Crypto.generateKeyPair(X509Utilities.DEFAULT_TLS_SIGNATURE_SCHEME))
    ...
    fun `serialize - deserialize X509CertPath`() {
        val rootCA = X509Utilities.createSelfSignedCACert(ALICE.name, Crypto.generateKeyPair(X509Utilities.DEFAULT_TLS_SIGNATURE_SCHEME))
        val certificate = X509Utilities.createTLSCert(BOB.name, BOB_PUBKEY, rootCA, emptyList(), emptyList())
        val expected = X509Utilities.createCertificatePath(rootCA.certificate, certificate, revocationEnabled = false)
        val serialized = expected.serialize(kryo).bytes

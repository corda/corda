...
sealed class FlowInitiator : Principal {
    ...
    data class OnStartup(val schedulingFlow: String) : FlowInitiator() {
        override fun getName(): String = schedulingFlow
    }
    /**
    ...
    val invocationContext: InvocationContext
        get() {
            ...
            when (this) {
                ...
                is FlowInitiator.OnStartup -> origin = InvocationOrigin.NodeStartup(this.schedulingFlow)
            }
            return InvocationContext.newInstance(
                    origin = origin,
                    actor = actor
            )
        }

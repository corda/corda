...
class FinalityFlow(val transaction: SignedTransaction,
    ...
    override fun call(): SignedTransaction {
        ...
        logger.info(transaction, "Started finalization, commands are ${commandDataTypes.joinToString(", ", "[", "]")}.")
        ...
        logger.info(transaction, "Broadcasting transaction to parties ${recipients.map { it.name }.joinToString(", ", "[", "]")}.")
        for (party in recipients) {
            logger.info(transaction, "Sending transaction to party ${party.name}.")
            val session = initiateFlow(party)
            subFlow(SendTransactionFlow(session, notarised))
            logger.info(transaction, "Party ${party.name} received the transaction.")
        }
        logger.info(transaction, "All parties received the transaction successfully.")
    ...
    private fun notariseAndRecord(): SignedTransaction {
        val notarised = if (needsNotarySignature(transaction)) {
            ...
            logger.info(transaction, "No need to notarise this transaction.")
            transaction
        }
        logger.info(transaction, "Recording transaction locally.")
        serviceHub.recordTransactions(notarised)
        logger.info(transaction, "Recorded transaction locally successfully.")
        return notarised
    }

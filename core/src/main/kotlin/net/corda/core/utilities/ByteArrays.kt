...
sealed class ByteSequence(private val _bytes: ByteArray, val offset: Int, val size: Int) : Comparable<ByteSequence> {
    ...
    fun slice(start: Int = 0, end: Int = size): ByteBuffer {
        require(start >= 0) {"Starting index must be greater or equal to 0"}
...
class OpaqueBytesSubSequence(override val bytes: ByteArray, offset: Int, size: Int) : ByteSequence(bytes, offset, size) {
    init {
        require(offset >= 0 && offset < bytes.size){"Offset must be greater or equal to 0, and less than backing array size"}
        require(size >= 0 && offset + size <= bytes.size){"SubSequence size must be greater or equal to 0, and less than backing array size"}
    }
}

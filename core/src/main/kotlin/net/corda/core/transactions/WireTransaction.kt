...
class WireTransaction(componentGroups: List<ComponentGroup>, val privacySalt: PrivacySalt, digestService: DigestService) : TraversableTransaction(componentGroups, digestService) {
    ...
    fun toLedgerTransaction(services: ServicesForResolution): LedgerTransaction {
        return toLedgerTransactionInternal(services.asInternal())
    }
    ...
    companion object {
         ...
         */
        @CordaInternal
        internal fun resolveStateRefBinaryComponent(stateRef: StateRef, verificationSupport: VerificationSupport): SerializedBytes<TransactionState<ContractState>>? {
            val coreTransaction = verificationSupport.getRequiredSignedTransaction(stateRef.txhash).coreTransaction
            // Get the network parameters from the tx or whatever the default params are.
            val params = verificationSupport.getNetworkParameters(coreTransaction.networkParametersHash)
                    ?: throw IllegalStateException("Should have been able to fetch parameters by this point")
            ...
            return when (coreTransaction) {
                ...
                is ContractUpgradeWireTransaction -> coreTransaction.resolveOutputComponent(verificationSupport, stateRef, params)
                is NotaryChangeWireTransaction -> coreTransaction.resolveOutputComponent(verificationSupport, stateRef)

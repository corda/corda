...
data class SignedTransaction(val txBits: SerializedBytes<CoreTransaction>,
    ...
    private fun resolveAndCheckNetworkParameters(resolutionSupport: StateResolutionSupport) {
        val txNetworkParameters = resolutionSupport.getNetworkParameters(networkParametersHash)
                ?: throw TransactionResolutionException(id)
        val groupedInputsAndRefs = (inputs + references).groupBy { it.txhash }
        groupedInputsAndRefs.forEach { entry ->
            val tx = resolutionSupport.getRequiredSignedTransaction(entry.key)
            val params = resolutionSupport.getNetworkParameters(tx.networkParametersHash) ?: throw TransactionResolutionException(id)

    ...
    inline fun <reified ReceiveType> receiveResultOrTiming(session: FlowSession): UntrustworthyData<ReceiveType> {
        while (true) {
            ...
            when (unwrapped) {
                ...
                else -> throw throw IllegalArgumentException("We were expecting a ${ReceiveType::class.java.name} or WaitTimeUpdate " +
                        "but we instead got a ${unwrapped.javaClass.name} ($unwrapped)")
            }
        }
    }

    open fun applyWaitTimeUpdate(session: FlowSession, update: WaitTimeUpdate) {
        logger.info("Counterparty [${session.counterparty}] is busy - TimedFlow $runId has been " +
                "asked to wait for an additional ${update.waitTime} seconds for completion.")

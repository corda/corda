...
import net.corda.core.internal.reflection.MethodClassifier.*
            ...
            field?.apply {
                if (!getterType.isSupertypeOf(genericReturnType))
                    throw NotSerializableException(
                            "Defined getter for parameter $name in $declaringClass returns type $getterType " +
        ...
        setter?.apply {
            ...
            field?.apply {
                if (!genericType.isSupertypeOf(setterType))
                    throw NotSerializableException(
                            "Defined setter for parameter $name in $declaringClass takes parameter of type $setterType " +
                                    "yet underlying type is $genericType")
            }

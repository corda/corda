package net.corda.core.internal.sgx
...
import net.corda.core.node.services.TransactionResolutionServices
 ...
 */
@CordaSerializable
data class FilteredBackChain(val transactionsById: Map<SecureHash, FilteredTransaction>) {


    /*
    fun resolveStateRef(txId: SecureHash, stateId: Int) {
        val tx =
    }
    */
    ...
    companion object {
        /*
        fun build(tx: WireTransaction, resolutionServices: TransactionResolutionServices): FilteredBackChain {
        }
        */
    }
}
...
class CheckedTxResolutionService(
        ...
        val resolvableBackChain: FilteredBackChain
    ...
    override val resolveStateRefAsSerialized: (StateRef) -> SerializedBytes<TransactionState<ContractState>>?
        get() = { stateRef: StateRef ->
            ...
            if (result != null) {
                 ...
                 */
                val (tx, id) = stateRef
                val inputFilteredTx = resolvableBackChain.transactionsById[tx]

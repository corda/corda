...
interface NodeInfo {
    ...
    /** Like [equals] but disregarding the [serial] field. */
    fun equalsWithoutSerial(that: NodeInfo?): Boolean
}

@CordaSerializable
data class NodeInfoImpl(override val addresses: List<NetworkHostAndPort>,
                        override val legalIdentitiesAndCerts: List<PartyAndCertificate>,
    ...
    override val legalIdentities: List<Party>
        get() {
            return _legalIdentities ?: legalIdentitiesAndCerts.map { it.party }.also { _legalIdentities = it }
        }

    override fun isLegalIdentity(party: Party): Boolean = party in legalIdentities
    override fun identityFromX500Name(name: CordaX500Name): Party = identityAndCertFromX500Name(name).party
    /**
    ...
    override fun equalsWithoutSerial(that: NodeInfo?) = that is NodeInfoImpl && copy(serial = 0L) == that.copy(serial = 0L)
}

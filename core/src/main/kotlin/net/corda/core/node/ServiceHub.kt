...
interface ServiceHub : ServicesForResolution {
    ...
    /**
     * Helper method to create an additional signature for a [WireTransaction].
     * @param wireTransaction The [WireTransaction] to which the signature will apply.
     * @param publicKey The [PublicKey] matching to a signing [PrivateKey] hosted in the node.
     * If the [PublicKey] is actually a [CompositeKey] the first leaf key found locally will be used for signing.
     * @return The [DigitalSignature.WithKey] generated by signing with the internally held [PrivateKey].
     */
    fun createSignature(wireTransaction: WireTransaction, publicKey: PublicKey): DigitalSignature.WithKey = keyManagementService.sign(wireTransaction.id.bytes, publicKey)
     ...
     * @return The DigitalSignature.WithKey generated by signing with the internally held identity PrivateKey.
     */
    fun createSignature(wireTransaction: WireTransaction): DigitalSignature.WithKey = createSignature(wireTransaction, legalIdentityKey)

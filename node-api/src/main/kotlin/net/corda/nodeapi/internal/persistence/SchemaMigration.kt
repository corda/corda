...
class SchemaMigration(
        ...
        private val databaseSchemaName: String? = null) {
     ...
     */
    fun runMigration(existingCheckpoints: Boolean, statusLogger: Logger? = null) {
        migrateOlderDatabaseToUseLiquibase(existingCheckpoints)
        doRunMigration(run = true, outputWriter = null, check = false, existingCheckpoints = existingCheckpoints, statusLogger = statusLogger)
    }
            ...
            mutex.withLock {
                ...
                val jarsToUnRunChanges: List<Pair<List<ChangeSet>, String>> = resourcesAndSourceInfo.map {
                    Liquibase(dynamicInclude, it.first, getLiquibaseDatabase(JdbcConnection(connection))).let { runner ->
                        runner.adhereSchemaNamespace(databaseSchemaName)
                        try {
                ...
                val runner = Liquibase(changeLog, null, getLiquibaseDatabase(JdbcConnection(connection))).apply {
                    adhereSchemaNamespace(databaseSchemaName)

...
class DemoBenchNodeInfoFilesCopier(scheduler: Scheduler = Schedulers.io()): Controller() {
     ...
     */
    fun addConfig(nodeConfig: NodeConfigWrapper) = nodeInfoFilesCopier.addConfig(nodeConfig.nodeDir)
     ...
     * one.
     */
    fun removeConfig(nodeConfig: NodeConfigWrapper) = nodeInfoFilesCopier.removeConfig(nodeConfig.nodeDir)

    fun reset() = nodeInfoFilesCopier.reset()
}
...
class DriverTests {
    ...
    companion object {
        ...
        private fun nodeMustBeUp(handleFuture: ListenableFuture<out NodeHandle>) = handleFuture.getOrThrow().apply {
            val hostAndPort = ArtemisMessagingComponent.toHostAndPort(nodeInfo.addresses.first())
        ...
        private fun nodeMustBeDown(handle: NodeHandle) {
            val hostAndPort = ArtemisMessagingComponent.toHostAndPort(handle.nodeInfo.addresses.first())
            // Check that the port is bound

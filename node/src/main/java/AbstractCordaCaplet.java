...
public class AbstractCordaCaplet extends Capsule {
    ...
    protected <T> T attribute(Map.Entry<String, T> attr) {
        ...
        if (ATTR_APP_CLASS_PATH == attr) {
            ...
            Stream.of("plugins", "dependencies").forEach(dirName -> {
                File dir = new File(dirName);
                dir.mkdir(); // XXX: Can't we do this somewhere else?
                ((List<Path>) value).add(dir.toPath().toAbsolutePath().resolve("*")); // Literal asterisk.
            });
        }
        return value;
    }

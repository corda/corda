...
class PersistentIdentityService(cacheFactory: NamedCacheFactory) : SingletonSerializeAsToken(), IdentityServiceInternal, NotaryUpdateListener {
    ...
    override fun verifyAndRegisterLegalIdentity(identity: PartyAndCertificate) {
        verifyAndRegisterIdentity(trustAnchor, identity, isLegalIdentity = true)
    }
    ...
    private fun verifyAndRegisterIdentity(trustAnchor: TrustAnchor, identity: PartyAndCertificate, isNewRandomIdentity: Boolean = false,
                                          isLegalIdentity: Boolean = false): PartyAndCertificate? {
        ...
        if (wellKnownCert != identity.certificate && !isNewRandomIdentity) {
            ...
            verifyAndRegisterIdentity(trustAnchor, PartyAndCertificate(firstPath), isNewRandomIdentity, isLegalIdentity)
        }
        return registerIdentity(identity, isNewRandomIdentity, isLegalIdentity)
    }

    private fun registerIdentity(identity: PartyAndCertificate, isNewRandomIdentity: Boolean, isLegalIdentity: Boolean):
            PartyAndCertificate? {

...
class InMemoryTransactionVerifierService(numberOfWorkers: Int, private val constraintsChecking: Boolean) : SingletonSerializeAsToken(), TransactionVerifierService, TransactionVerifierServiceInternal, AutoCloseable {
    ...
    override fun verify(transaction: LedgerTransaction, extraAttachments: List<Attachment>): CordaFuture<Unit> {
        return openFuture<Unit>().apply {
            capture {
                val verifier = transaction.prepareVerify(extraAttachments, constraintsChecking)

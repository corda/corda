...
internal object V1NodeConfigurationSpec : Configuration.Specification<NodeConfiguration>("NodeConfiguration") {
    ...
    private val useTestClock by boolean().optional().withDefaultValue(false)
    private val lazyBridgeStart by boolean()
    private val detectPublicIp by boolean().optional().withDefaultValue(true)
    private val sshd by nested(SSHDConfigurationSpec).optional()
    private val database by nested(DatabaseConfigSpec)
    private val noLocalShell by boolean().optional().withDefaultValue(false)
    private val transactionCacheSizeBytes by long().optional().withDefaultValue(NodeConfiguration.defaultTransactionCacheSize)
    ...
    private val flowMonitorSuspensionLoggingThresholdMillis by duration().optional().withDefaultValue(DEFAULT_FLOW_MONITOR_SUSPENSION_LOGGING_THRESHOLD_MILLIS)

...
interface ScheduledFlowRepository {
    fun delete(key: StateRef)
    fun merge(value: ScheduledStateRef)
    fun getLatest() : Pair<StateRef, ScheduledStateRef>?
}
...
class PersistentScheduledFlowRepository(val database: CordaPersistence): ScheduledFlowRepository {
    ...
    private fun fromPersistentEntity(scheduledStateRecord: NodeSchedulerService.PersistentScheduledState): Pair<StateRef, ScheduledStateRef> {
        ...
        val index = scheduledStateRecord.output.index ?: throw IllegalStateException("DB returned null SecureHash index")
    ...
    override fun getLatest() : Pair<StateRef, ScheduledStateRef>? {
        return database.transaction {
            ...
            session.createQuery(criteriaQuery).setFirstResult(0).setMaxResults(1)
                    .resultList.map { e -> fromPersistentEntity(e as NodeSchedulerService.PersistentScheduledState) }.firstOrNull()
        }
    }
}
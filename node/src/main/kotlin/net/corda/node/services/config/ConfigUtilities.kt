...
fun NodeConfiguration.configureWithDevSSLCertificate() = signingAndP2pConfiguration().configureDevKeyAndTrustStores(myLegalName)

// TODO Move this to KeyStoreConfigHelpers
// TODO sollecitom refactor
fun Pair<CertificateStoreSupplier, SSLConfiguration>.configureDevKeyAndTrustStores(myLegalName: CordaX500Name) {
    second.certificatesDirectory.createDirectories()
    if (!second.trustStoreFile.exists()) {
        loadKeyStore(javaClass.classLoader.getResourceAsStream("certificates/$DEV_CA_TRUST_STORE_FILE"), DEV_CA_TRUST_STORE_PASS).save(second.trustStoreFile, second.trustStorePassword)
    }
    if (!second.sslKeystore.exists() || first.getOptional() == null) {
        // TODO sollecitom split this for signing and p2p
        val (nodeKeyStore) = createDevKeyStores(myLegalName)

        // Move distributed service composite key (generated by IdentityGenerator.generateToDisk) to keystore if exists.
        val distributedServiceKeystore = second.certificatesDirectory / "distributedService.jks"

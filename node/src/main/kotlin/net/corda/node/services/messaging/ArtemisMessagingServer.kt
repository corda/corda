...
class ArtemisMessagingServer(override val config: NodeConfiguration,
                             val messagingHostPort: HostAndPort,
    ...
    private fun configureAndStartServer() {
        ...
        printBasicNodeInfo("Node ${this.config.myLegalName} listening on address", messagingHostPort.toString())
        if (rpcHostPort != null) {
    ...
    private fun createArtemisConfig(): Configuration = ConfigurationImpl().apply {
        ...
        val acceptors = mutableSetOf(tcpTransport(Inbound, "0.0.0.0", messagingHostPort.port))
        if (rpcHostPort != null) {

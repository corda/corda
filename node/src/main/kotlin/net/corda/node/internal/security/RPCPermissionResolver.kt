...
import net.corda.ext.api.rpc.proxies.RpcAuthHelper.INTERFACE_SEPARATOR
...
internal object RPCPermissionResolver : PermissionResolver {
        ...
        val membersPairs = interfaceClass.members.flatMap { member ->
            when(member) {
                is KFunction -> {
                    ...
                    if (method != null) {
                        processMethod(method, member.annotations, interfaceClass)
                    } else {
    ...
    private fun processMethod(method: Method, annotations: Iterable<Annotation>, interfaceClass: KClass<out Any>): List<Pair<String, String>> {
        ...
        processAnnotations(method, annotations) + // Groups as specified by annotations
        listOf(methodFullName.toLowerCase() to methodFullName) // Full method names individually
    }

    private fun processAnnotations(method: Method, annotations: Iterable<Annotation>) : List<Pair<String, String>> {
        val rpcPermGroups = annotations.mapNotNull { it as? RpcPermissionGroup }
        val permGroupsNames = rpcPermGroups.flatMap { it.value.asList() }.toSet()
        return permGroupsNames.map {
            val groupKey = methodFullName(method.declaringClass, it).toLowerCase()
            groupKey to methodFullName(method)
        }
    }
}
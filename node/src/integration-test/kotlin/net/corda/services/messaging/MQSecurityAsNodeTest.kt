...
class MQSecurityAsNodeTest : P2PMQSecurityTest() {
    ...
    fun `login with invalid certificate chain`() {
        ...
        signingCertStore.get(createNew = true).update {
            setPrivateKey(X509Utilities.CORDA_CLIENT_CA, clientKeyPair.private, listOf(clientCACert, DEV_INTERMEDIATE_CA.certificate, DEV_ROOT_CA.certificate), signingCertStore.keyPassword)
        }

        p2pSslConfig.keyStore.get(createNew = true).update {
            setPrivateKey(X509Utilities.CORDA_CLIENT_TLS, tlsKeyPair.private, listOf(clientTLSCert, clientCACert, DEV_INTERMEDIATE_CA.certificate, DEV_ROOT_CA.certificate), p2pSslConfig.keyStore.keyPassword)
        }

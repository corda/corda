...
class StateMachineGeneralErrorHandlingTest : StateMachineErrorHandlingTest() {
     ...
     */
    @Test(timeout = 300_000)
    fun `error saving a flow with HOSPITALIZED status the flow retries and updates the status in the database`() {
        startDriver {
            val alice = createBytemanNode(ALICE_NAME)
            ...
            assertFailsWith<TimeoutException> {
                aliceClient.startFlow(StateMachineErrorHandlingTest::ThrowAHospitalizeErrorFlow).returnValue.getOrThrow(40.seconds)
            }
            val output = getBytemanOutput(alice)
    ...
    fun `responding flow - error during transition with CommitTransaction action that occurs when completing a flow and deleting its checkpoint will retry and complete successfully`() {
        startDriver {
            ...
            assertEquals(
                1,
                charlieClient.startFlow(StateMachineErrorHandlingTest::GetNumberOfCheckpointsFlow).returnValue.get()
            )
        }
    }
}
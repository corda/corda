...
abstract class AbstractNetworkMapServiceTest<out S : AbstractNetworkMapService> {
    ...
    lateinit var bankA: MockNode

    @Before
    fun setup() {
        network = MockNetwork(defaultFactory = nodeFactory)
        network.createTwoNodes(firstNodeName = "map service", secondNodeName = DUMMY_BANK_A.name).apply {
    ...
    fun `all nodes register themselves`() {
        ...
        assertThat(bankA.fetchMap()).containsOnly(Added(mapServiceNode), Added(bankA))
        assertThat(bankA.identityQuery()).isEqualTo(bankA.info)
        assertThat(mapServiceNode.identityQuery()).isEqualTo(mapServiceNode.info)
    }

    @Test
    fun `re-register the same node`() {
        val response = bankA.registration(ADD)
        swizzle()
        assertThat(response.getOrThrow().error).isNull()
        assertThat(bankA.fetchMap()).containsOnly(Added(mapServiceNode), Added(bankA))  // Confirm it's a no-op
    }

    @Test
    fun `re-register with smaller serial value`() {
        val response = bankA.registration(ADD, serial = 1)
        swizzle()
    ...
    fun `de-register node`() {
        val response = bankA.registration(REMOVE)
        swizzle()
        assertThat(response.getOrThrow().error).isNull()
        assertThat(bankA.fetchMap()).containsOnly(Added(mapServiceNode), Removed(bankA))
        swizzle()
        assertThat(bankA.identityQuery()).isNull()
        assertThat(mapServiceNode.identityQuery()).isEqualTo(mapServiceNode.info)
    }

    @Test
    fun `de-register same node again`() {
        bankA.registration(REMOVE)
        val response = bankA.registration(REMOVE)
        swizzle()
        assertThat(response.getOrThrow().error).isNotNull()  // Make sure send error message is sent back
        assertThat(bankA.fetchMap()).containsOnly(Added(mapServiceNode), Removed(bankA))
    ...
    fun `de-register unknown node`() {
        val bankB = newNodeSeparateFromNetworkMap(DUMMY_BANK_B.name)
        val response = bankB.registration(REMOVE)
        swizzle()
        assertThat(response.getOrThrow().error).isNotNull()  // Make sure send error message is sent back
        assertThat(bankA.fetchMap()).containsOnly(Added(mapServiceNode), Added(bankA))
    }
    ...
    fun `subscribed while new node registers`() {
        val updates = bankA.subscribe()
        swizzle()
        val bankB = addNewNodeToNetworkMap(DUMMY_BANK_B.name)
        swizzle()
        val update = updates.single()
        assertThat(update.mapVersion).isEqualTo(networkMapService.mapVersion)
        assertThat(update.wireReg.verified().toChanged()).isEqualTo(Added(bankB.info))
    }
    ...
    fun `subscribed while node de-registers`() {
        val bankB = addNewNodeToNetworkMap(DUMMY_BANK_B.name)
        val updates = bankA.subscribe()
        bankB.registration(REMOVE)
        swizzle()
        assertThat(updates.map { it.wireReg.verified().toChanged() }).containsOnly(Removed(bankB.info))
    }
    ...
    fun unsubscribe() {
        ...
        val bankB = addNewNodeToNetworkMap(DUMMY_BANK_B.name)
        bankA.unsubscribe()
        addNewNodeToNetworkMap(DUMMY_BANK_C.name)
        swizzle()
        assertThat(updates.map { it.wireReg.verified().toChanged() }).containsOnly(Added(bankB.info))
    }
    ...
    fun `surpass unacknowledged update limit`() {
        ...
        repeat(networkMapService.maxUnacknowledgedUpdates) {
            bankB.registration(ADD, serial = ++serial)
            swizzle()
        }

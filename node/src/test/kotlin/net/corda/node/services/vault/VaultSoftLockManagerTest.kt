...
class VaultSoftLockManagerTest {
    ...
    private val mockNet = MockNetwork(cordappPackages = listOf(ContractImpl::class.packageName), defaultFactory = { args ->
        object : MockNetwork.MockNode(args) {
            private fun makeVaultService(lhf: LazyHubFactory): VaultServiceInternal {
                val realVault = lhf.child().let { lh ->
            ...
            override fun configure(lh: MutableLazyHub) {
                super.configure(lh)
                lh.factory(VaultServiceInternal::class, this::makeVaultService)
            }
        }
    })

...
class TwoPartyTradeFlowTests(private val anonymous: Boolean) {
    ...
    fun `trade cash for commercial paper`() {
        ...
        ledger(MockServices(cordappPackages)) {
            ...
            bobNode.database.transaction {
                VaultFiller(bobNode.services, Pair(DUMMY_NOTARY, DUMMY_NOTARY_KEY), notary, ::Random).fillWithSomeTestCash(2000.DOLLARS, bankNode.services, 3, 10, cashIssuer)
            }
    ...
    fun `trade cash for commercial paper fails using soft locking`() {
        ...
        ledger(MockServices(cordappPackages)) {
            ...
            val cashStates = bobNode.database.transaction {
                VaultFiller(bobNode.services, Pair(DUMMY_NOTARY, DUMMY_NOTARY_KEY), notary, ::Random).fillWithSomeTestCash(2000.DOLLARS, bankNode.services, 3, issuer)
            }
    ...
    fun `shutdown and restore`() {
        ...
        ledger(MockServices(cordappPackages)) {
            ...
            bobNode.database.transaction {
                VaultFiller(bobNode.services, Pair(DUMMY_NOTARY, DUMMY_NOTARY_KEY), notary, ::Random).fillWithSomeTestCash(2000.DOLLARS, bankNode.services, 3, 10, issuer)
            }
            val alicesFakePaper = aliceNode.database.transaction {
                fillUpForSeller(false, issuer, alice,
                        1200.DOLLARS `issued by` bank.ref(0), null, notary).second

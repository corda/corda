...
class InMemoryMessagingTests {
    ...
    fun basics() {
        ...
        node1.network.send(node1.network.createMessage("test.topic", data = bits, context = testContext()), node2.network.myAddress)
    ...
    fun broadcast() {
        ...
        node1.network.send(node2.network.createMessage("test.topic", data = bits, context = testContext()), mockNet.messagingNetwork.everyoneOnline)
        mockNet.runNetwork(rounds = 1)
    ...
    fun `skip unhandled messages`() {
        ...
        val invalidMessage = node2.network.createMessage("invalid_message", data = ByteArray(0), context = testContext())
        val validMessage = node2.network.createMessage("valid_message", data = ByteArray(0), context = testContext())
        ...
        val invalidMessage2 = node2.network.createMessage("invalid_message", data = ByteArray(0), context = testContext())
        val validMessage2 = node2.network.createMessage("valid_message", data = ByteArray(0), context = testContext())
